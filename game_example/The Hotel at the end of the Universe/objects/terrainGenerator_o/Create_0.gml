islandType=choose_weight(["roof",4,"noroof",8,"doubleroof",6,"litroof",16,"litwalls",12])
...
for (var i=0; i<real_island_size; i++){
	...
	depthOfPoint=10//(island_depth+irandom_range(0,3))*power(2.71,-abs((i-real_island_size/2)/sqrt(real_island_size)))
	last_y=newY
...
for (var i=0; i<surface_point_num; i++){
	...
	if irandom(enemyChance)==1{
		var en=instance_create_layer(
			...
			new_y-24,
			"enemies",
			choose_weight([zombie_o,4,zombieBird_o,4])
		)
		if en.object_index==zombieBird_o en.y-=16
	}
	if last_y!=new_y{
		while last_y!=new_y{
	...
	if (irandom_range(0,8)==8) and surface_points[i][0]<room_width and i==round((surface_point_num-1)/2){
		instance_create_layer(surface_points[i][0]+tile_size/2,min(new_y,last_y)-tile_size/2,"enemies",spawner_o)
	}
	last_y=new_y
	if max_y<last_y max_y=last_y
	if fenceNum>0{
		fenceNum-=1
		tilemap_set_at_pixel(decor_id,34,surface_points[i][0],new_y-16-16*(tilemap_get_at_pixel(tilemap_id,surface_points[i][0],new_y-16)==2))
